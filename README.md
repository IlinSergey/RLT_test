...
Для запуска проекта необходимо создать файл config.py где необходимо прописать переменные(см. образец `example.config.py`)

Можно запустить бота в Docker,
В директории с проектом расположен Dockerfile для создания образа бота. В таком случае необходимо убедиться в правильности подключения бота к БД.
Например при запущенном сервисе Mongo на хосте (на linux) для подключения бота к бд - нужно запускать контейнер командой `docker run -d --network host <image name>`.
